class Posts{
    ...
    static addPost = (post , callback) =>{
        ...
        buildfire.auth.getCurrentUser((err, currentUser) =>{
            ...
            buildfire.publicData.insert(post, Posts.TAG, (err, rPost) =>{
    ...
    static deletePublicPost = (id, callback) =>{
        buildfire.publicData.getById(id, Posts.TAG, (err,r) =>{
            if(err) return callback({code: errorsList.ERROR_404,message:"Couldn't find post with this ID"});
            buildfire.publicData.delete(id, Posts.TAG, (err, r) =>{
    ...
    static deletePost = (filter, callback) =>{
        ...
        buildfire.auth.getCurrentUser((err, currentUser) =>{
            if(err || !currentUser) return callback({code: errorsList.ERROR_401,message:"Must be logged in"});
            buildfire.publicData.search({filter:{$and:[{...filter}]},sort:{createdOn: -1} }, Posts.TAG, (err, r) =>{
                ...
                r.forEach(p =>{
                    ...
                    buildfire.publicData.delete(p.id, Posts.TAG, (err, r) =>{
                        if(err) return console.error(err);
                        callback(r);
                    })
    ...
    static getPosts = (options , callback) =>{
        ...
        buildfire.auth.getCurrentUser((err, currentUser) =>{
            if(err || !currentUser){
                buildfire.publicData.search({filter : {$or: tempArray} , skip : options?.skip || 0 , limit : options?.limit || 6, sort:options?.sort ? options.sort : {createdOn : -1}} , Posts.TAG , (e , r) =>{
                ...
                Follows.getUserFollowData((err , resp) =>{            
                    ...
                    buildfire.publicData.search({filter : {$or: tempArray} , skip : options?.skip || 0 , limit : options?.limit || 6 , sort:{createdOn : -1}} , Posts.TAG , (e , r) =>{
                        e ? callback(e , null) : callback(null , r);
                    })                
                })
    ...
    static getNewPosts = (options, callback) =>{
        ...
        buildfire.auth.getCurrentUser((err, currentUser) =>{
            if(err || !currentUser) {
                buildfire.publicData.search({filter :{$and:[{$or:tempArray},{"$json.createdOn":{$gt:new Date(options.lastPostDate)}}]} , skip : options?.skip || 0 , limit : options?.limit || 6 ,page: options?.page || 0, pageSize: options?.pageSize, sort:options?.sort ? options.sort : {createdOn : -1}} , Posts.TAG , (e , r) =>{    
                    e ? callback(e , null) : callback(null , r)
                })
            }
            else{
                Follows.getUserFollowData((err , resp) =>{            
                    if(err || !resp){
                        buildfire.publicData.search({filter : {$and:[{$or: tempArray},{"$json.createdOn":{$lt:new Date(),$gt:new Date(options.lastPostDate)}}]} , skip : options?.skip ||  0 , limit : options?.limit || 6 ,page: options?.page || 0, pageSize: options?.pageSize, sort:options?.sort ? options.sort : {createdOn : -1}} , Posts.TAG , (e , r) =>{
                            e ? callback(e , null) : callback(null , r)
                        })
                    }
                    resp?.followedUsers && resp.followedUsers.forEach(id => tempArray.push({"_buildfire.index.array1.string1" : `userId_${id}`}));
    ...
    static updatePost = (id , post , callback) =>{
        ...
        buildfire.auth.getCurrentUser((err, currentUser) => {
            if(err || !currentUser) return callback({code: errorsList.ERROR_401,message:"Must be logged in"});
            buildfire.publicData.getById(id , Posts.TAG , (e , r) => {
                if(e) return callback({code:errorsList.ERROR_404,message:"Couldn't find matching data"});
                    ...
                    buildfire.publicData.update( id, {...r.data , postText : post?.postText || "", postImages : post?.postImages || [] , isPublic : post?.isPublic || false}, Posts.TAG, (e , r) => {
    ...
    static updatePublicPost = (id , post , callback) =>{
        ...
        buildfire.publicData.getById(id , Posts.TAG , (e , r) => {
            if(e) return callback({code:errorsList.ERROR_404,message:"Couldn't find matching data"});
                ...
                buildfire.publicData.update( id, {...r.data , postText : post.postText, displayName: post.postTitle}, Posts.TAG, (e , r) => {
                    if(e) return callback({code:errorsList.ERROR_400,message:e});
                    callback(null , r);
                    // buildfire.analytics.trackAction(analyticKeys.POST_postD.key);
                });
