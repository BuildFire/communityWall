class Follows {
  ...
  static followUser = (fUserId, callback) => {
    ...
    buildfire.auth.getCurrentUser((e,currentUser) => {
      ...
      buildfire.publicData.search({ filter: { "_buildfire.index.string1": currentUser._id } },Follows.TAG, (e,r) =>{
        if(e) return callback(e);
        if(r.length == 0) {
          buildfire.publicData.insert(Follows.createFollowData(currentUser, fUserId), Follows.TAG, (e, r) => {
          ...
          buildfire.publicData.update(r[0].id, {...r[0].data,followedUsers: [...r[0].data.followedUsers, fUserId] }, Follows.TAG, (e, r) =>{
              if (e) return callback(e);
              callback(null, r);
          });
        }
    ...
    buildfire.auth.getCurrentUser((e,currentUser) => {
      ...
      buildfire.publicData.search({ filter: { "_buildfire.index.string1": currentUser._id } },Follows.TAG, (e,r) =>{
        ...
        if(r.length == 0) return callback("Not following this user");
          ...
          buildfire.publicData.update(r[0].id, {...r[0].data,followedUsers: newFollowedUsers }, Follows.TAG, (e, r) =>{
              if (e) return callback(e);
              callback(null, r);
          });
        }
  ...
  static followPlugin = (callback) =>{
    buildfire.auth.getCurrentUser((e, currentUser) => {
      ...
      buildfire.getContext((err, context) => {
        ...
        buildfire.publicData.search({filter: { "_buildfire.index.string1": currentUser._id }},Follows.TAG , (err,r) =>{
          if(e || !r) return callback({code: errorsList.ERROR_404, message: "Couldn't find matching data"});
          else if(r.length == 0){
            buildfire.publicData.insert(Follows.createFollowData(currentUser,null,instanceId),Follows.TAG,(e, r) => {
                if (e) return callback(e);
                else return callback(null, r);
            });
          }
            ...
            buildfire.publicData.update(r[0].id,{...r[0].data, followedPlugins: [...r[0].data.followedPlugins, instanceId]}, Follows.TAG, (e, r) => {
      ...
      buildfire.getContext((err, context) => {
        ...
        buildfire.publicData.search({filter: { "_buildfire.index.string1": currentUser._id }},Follows.TAG , (e,r) =>{
          ...
          if (index >= 0) {
            ...
            buildfire.publicData.update(r[0].id, obj, Follows.TAG, (e, r) => {
              if (e) return callback({code: errorsList.ERROR_404, message: "Couldn't find matching data"});
              callback(r);
            });
          } 
